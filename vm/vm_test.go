...
import (
	"fmt"
	"github.com/eiji03aero/monkey/ast"
	"github.com/eiji03aero/monkey/compiler"
	"github.com/eiji03aero/monkey/lexer"
	"github.com/eiji03aero/monkey/object"
	"github.com/eiji03aero/monkey/parser"
	"testing"
)
...
func testExpectedObject(
	...
	case []int:
		...
		if len(array.Elements) != len(expected) {
			t.Errorf("array.Elements has wrong length. want=%d, got=%d", len(array.Elements), len(expected))
			return
		}
	...
	case map[object.HashKey]int64:
		...
		for expectedKey, expectedValue := range expected {
			...
			if err != nil {
				t.Errorf("testsIntegerObject railed: %s", err)
			}
		}
...
func testIntegerObject(expected int64, actual object.Object) error {
	...
	if result.Value != expected {
		return fmt.Errorf("result.Value has wrong value. want=%d, got=%d", result.Value, expected)
	}
...
func testStringObject(expected string, actual object.Object) error {
	...
	if result.Value != expected {
		return fmt.Errorf("result.Value has wrong value. want=%q, got=%q", result.Value, expected)
	}
...
func testBooleanObject(expected bool, actual object.Object) error {
	...
	if result.Value != expected {
		return fmt.Errorf("result.Value has wrong value. want=%t, got=%t", result.Value, expected)
	}
